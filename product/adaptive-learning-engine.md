...
**对本报告结论的影响范围**：§5"与批阅系统产品线的关系"一节在**知识资产层面**的判断不受影响
（批阅系统与自适应学习引擎在题库/知识点资产上确实未见共享，这次发现的集成是自适应学习引擎内部
两个子仓库之间关于知识资产的集成，不涉及批阅系统）；但 §5 在**证据数据层面**的判断已于本轮核查
更正为"存在单向摄入管道"，二者是不同维度、互不矛盾，详见 §5 的完整更正说明。§2.3"三仓定位"表格中"v2 是生产主线"的判断本身也不受影响（那是基于 git 活跃度和
...
归并错误模型 + 建立干扰项映射。其中 `rubric-svc` 导入的评分结构快照**真实来自批阅系统**（自称
"上游防腐层"，只读镜像，详见 §5.2），是知识治理闭环与批阅系统之间已有实现的证据级耦合点；除此
之外，这条闭环与 §3.3.6 的 `platform-data-foundation-service` 在业务概念（知识点、题目资产）上
高度相似，但两者代码/数据仍完全独立，尚未见已实现的协同（资产层耦合判断详见 §5.3）。
...
> **更正说明（2026-08-20）**：此前版本判断"未发现代码或数据层面的直接耦合"——经本轮深入核查
> `adaptive-learning-engine-lab/engine/pipelines/ingest_grading.py` 与 `services/rubric_svc/`
> 的代码，**这一结论不够准确，予以更正**。两条产品线在组织与代码仓库层面确实独立，但存在一条
> **真实的单向数据摄入管道**：批阅系统是 ALE 的上游证据来源之一，而非完全零交集。

### 5.1 组织与代码层面：独立产品线，无组织耦合

两条产品线是完全独立的目录、独立的 git 历史、独立的技术栈选型（批阅线是 FastAPI+Vue3.5+MySQL，
自适应学习线是 FastAPI+PostgreSQL+React19）；商业模式独立签约——批阅系统已有 5 所付费学校 + 10
省 20+ 所试点协议、是已量产的产品，而 ALE 的自适应推题核心仍在甘肃定西一地试点验证中（见 §1.2）。
`platform-data-foundation-service` 的 AGENTS.md 明确写着"不介入批阅系统的题库/知识树"——这是刻意
划清知识资产边界的产品决策。

### 5.2 数据流向：批阅系统是 ALE 的上游证据来源，单向、只读、经防腐层隔离

以下是本轮核查发现的真实代码证据，此前报告遗漏：

- **`rubric-svc` 自称"B13 上游防腐层"**（`services/rubric_svc/app.py` 模块注释原文）：卷子已由
  教师或批阅硬件批过，rubric-svc 需要的是把"第 2 步扣了 4 分"翻译成"卡在哪一步"，是**上游批阅侧
  评分结构的本地只读镜像**——契约层面故意不提供修订端点也不提供 DELETE（"覆盖会让历史作答的
  `rubric_version` 指向一份被改过的评分结构，步骤级诊断的溯源就断了"），只有 GET 与 POST 三条
  路由，PUT/PATCH/DELETE 一律 405。
- **`pipelines/ingest_grading.py` 管线**：真实调用批阅系统 API 端点
  `/grade-server/api/v1/learning/grading-results` 拉取批阅结果，转换后经 `evidence-svc` 的
  真实写路径落库为 `attempt` 记录——`exposure_src` 显式标记为 `exam`（区别于引擎自己推送的
  `engine_push`，代码注释强调"这批是考试通道，不是引擎推送。写错会污染"）；`source_task_id`
  直接复用批阅系统的 `task_id` 作为幂等键，防止重跑产生重复证据。
- **产品体验层面的接续关系**：对外产品理念材料（§1.1）描述的"一次讲评闭环"——"扫描批阅（老师
  照常批改）→ 系统找出关键问题 → 生成分层练习"——批阅是第一步，ALE 承接批阅结果做诊断和推题是
  第二步，两者在业务叙事上是**前后接续**关系。

### 5.3 知识资产层面：目前仍无共享

`platform-data-foundation-service` 的 P0.2 基线数据来源标注为 `adaptive_engine_teacher_analysis_v7`——
这个命名指向"自适应引擎教师分析"场景而非批阅系统，说明知识点/题库资产这条线的种子数据来自 ALE
自己的历史积累，不是复用批阅系统的题库；全文搜索也未发现批阅系统读取 ALE 的知识点/推题能力，或
ALE 反向为批阅系统提供任何服务——**资产层是真正意义上的零耦合**，与 §5.2 的证据层耦合是两个独立
的问题维度。

### 5.4 准确的关系总结

**两者是独立产品线、独立商业实体，但存在一条明确的单向数据管道：批阅系统产出的评分结果和评分
结构，经过 ALE 侧的防腐层（`rubric-svc`）和摄入管线（`ingest_grading.py`）转换后，成为 ALE 六轴
诊断和自适应推题的输入证据之一；这条关系是"批阅先发生、ALE 后消费"的流水线接续关系，而非"两个
系统互相调用/共享数据库/共享知识资产"的深度集成。**

**注意区分三个不同层级的关系问题，避免混淆**：
1. 批阅系统 ↔ ALE（本节，产品线级别）——结论：证据层单向摄入，资产层零耦合
2. `platform-data-foundation-service` ↔ 自适应学习引擎早期主线（§2.4，ALE 产品线**内部**两个
   子仓库之间）——结论：已有真实生产集成
3. 是否应该、以及如何让批阅系统也接入 `platform-data-foundation-service` 这套知识点/题库资产
   底座，仍是一个尚未见到裁定的产品级问题，建议作为后续与用户确认的问题项。
