...
- [self-learning-engine.md](self-learning-engine.md) — 自适应学习引擎（高中全学科自适应教学系统）产品线调研：与批阅系统完全独立的第二条产品线，三个并行子仓库现状（`education-data-foundation` 生产级公司数据底座 · `自适应学习引擎` 早期主线 45 份现行方案文档+无远程本地 engine · `自适应学习引擎-全量实验室-v2` 后期主线 163 提交/68 PR 九微服务+teacher-web 前端）；§4 新增顶层 45 份方案文档按主题分组（唯一入口与总纲/核心架构与事实基线/PRD与流程修复/知识点题库资产/实测证据与试产报告/产品化上线试点/用户市场支线/协作编排技术选型审计共 8 组）；九微服务接口契约与多 Agent 编排状态机详解；转引项目内部 2026-08-18 审计的五个未处置风险（含本地代码无远程备份、明文凭据 8 天未轮换等不可逆风险）；§7 补充正确的三仓工作指引（v2 为生产主线/foundation 为数据集与处理管道/早期主线仅作 PRD 参考）
